  ...
  if(is_ptr(va)) {
	...
	/*        if (sz > 50) printf("Moving value %p with tag %ld with %d fields\n", (void*)v, hd, sz); */
        ...
        for(i = 0; i < sz; i++) {
          ...
          Field(newp, i) = Field(v, i);
        }
        Hd_val(v) = 0;
	Field(v, 0) = ptr_to_int_or_ptr((void *)newp);
	*p = ptr_to_int_or_ptr((void *)newp);
        /* printf("newp %lld\n", newp); */
        /* if (*p == 73832) printf("Found it\n"); */
        if (depth>0)
          for (i=0; i<sz; i++)
            forward(from_start, from_limit, next, &Field(newp,i), depth-1);
      }
    }
  }
}
...
void *export_from_root(struct thread_info *ti, value root) {
